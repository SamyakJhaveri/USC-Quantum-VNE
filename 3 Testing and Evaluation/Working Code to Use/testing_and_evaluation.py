...
def get_current_solution_sampleset_evaluation_data(sampleset):
    ...
    print("QPU Access Time (QPU time, in microseconds, used by the hybrid solver., i.e."
        ...
        "\nTotal time in QPU):{} microseconds".format(sampleset.info["qpu_access_time"]))
...
def get_solver_sampler_information(sampler):
    ...
    print("\nLeap Hybrid DQM Sampler Properties are:{}".format(sampler.properties))
    print("\nLeap Hybrid DQM Sampler Solver Parameters are:{}".format(sampler.parameters))
    # Category
